...
class BulkLicensesController extends Controller
{
     ...
     */
    protected function conditionallyAddItem($field) : BulkLicensesController
    {
...
//                                    $asset->{$field->db_column} = $this->update_array[$field->db_column];
    ...
    public function destroy(Request $request) : RedirectResponse
    {
        ...
        $bulk_back_url = route('license.index');
        if ($request->session()->has('bulk_back_url')) {
            $bulk_back_url = $request->session()->pull('bulk_back_url');
        }

        $licenses = License::find($request->get('ids'));

        if ($licenses->assigned_seats_count == 0 && $request->filled('ids')) {
